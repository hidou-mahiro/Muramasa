...
class MutualSelfAttentionControl(AttentionBase):
    ...
    def attn_batch(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        b = q.shape[0] // num_heads     # number of batches
    ...
    def forward(self, q, k, v, sim, attn, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        qa, qb, qc, qd = q.chunk(4)
        ka, kb, kc, kd = k.chunk(4)
        va, vb, vc, vd = v.chunk(4)
        attna, attnb, attnc, attnd = attn.chunk(4)
        
        '''
        kb = torch.cat([ka, kb], dim=0)
        kc = torch.cat([kb, kc], dim=0)
        kd = torch.cat([kc, kd], dim=0)
        
        vb = torch.cat([va, vb], dim=0)
        vc = torch.cat([vb, vc], dim=0)
        vd = torch.cat([vc, vd], dim=0)
        
        attnb = torch.cat([attna, attnb], dim=0)
        attnc = torch.cat([attnb, attnc], dim=0)
        attnd = torch.cat([attnc, attnd], dim=0)
        
        out_a = self.attn_batch(qa, ka[:num_heads], va[:num_heads], sim[:num_heads], attna, is_cross, place_in_unet, num_heads, **kwargs)
        out_b = self.attn_batch(qb, kb[:2*num_heads], vb[:2*num_heads], sim[:2*num_heads], attnb, is_cross, place_in_unet, num_heads, **kwargs)
        out_c = self.attn_batch(qc, kc[:3*num_heads], vc[:3*num_heads], sim[:3*num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs)
        out_d = self.attn_batch(qd, kd[:4*num_heads], vd[:4*num_heads], sim[:4*num_heads], attnd, is_cross, place_in_unet, num_heads, **kwargs)
        out = torch.cat([out_a, out_b, out_c, out_d], dim=0)
        '''
        out_a = self.attn_batch(qa, ka[:num_heads], va[:num_heads], sim[:num_heads], attna, is_cross, place_in_unet, num_heads, **kwargs)
        out_b = self.attn_batch(qb, kb[:num_heads], vb[:num_heads], sim[:num_heads], attnb, is_cross, place_in_unet, num_heads, **kwargs)
        out_c = self.attn_batch(qc, kc[:num_heads], vc[:num_heads], sim[:num_heads], attnc, is_cross, place_in_unet, num_heads, **kwargs)
        out_d = self.attn_batch(qd, kd[:num_heads], vd[:num_heads], sim[:num_heads], attnd, is_cross, place_in_unet, num_heads, **kwargs)
        out = torch.cat([out_a, out_b, out_c, out_d], dim=0)
